...
static void setup_timer(u64 seconds)
{
	...
	data_high = ioread32(tp->base_addr + XIL_AXI_TIMER_TCSR1_OFFSET);
	iowrite32(data_high | XIL_AXI_TIMER_CSR_LOAD_MASK,
			tp->base_addr + XIL_AXI_TIMER_TCSR1_OFFSET);
	data_low = ioread32(tp->base_addr + XIL_AXI_TIMER_TCSR0_OFFSET);
	iowrite32(data_low | XIL_AXI_TIMER_CSR_LOAD_MASK,
			tp->base_addr + XIL_AXI_TIMER_TCSR0_OFFSET);
	...
	// Enable interrupts, cascade and generate mode and downcounting rest should be zero
...
ssize_t timer_write(struct file *pfile, const char __user *buffer, size_t length, loff_t *offset) 
{
	...
	else if(strncmp(buff, "stop", 4) == 0)
	{
		if(flag_stop == 0)
		{
			flag_stop = 1;
			flag_start = 0;
			printk(KERN_INFO "Timer is stopping");
			stop_timer();
		}
		else
